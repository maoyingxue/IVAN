...
bool Geometry::isCrossLine(list<Blob> &blobs, Point start, Point end, int &cnt, int &index)
{
    ...
    for (auto &blob : blobs)
    {
        ...
        if (blob.getCenter().size() >= 2 && !blobIsCounted[index] && blob.getIsCurrentBlob())
        {
            ...
            if (Geometry::isSegmentCross(start, end, preFrameCenter, curFrameCenter))
            {
                cnt++;
                blob.changeIsCountedToTrue(index);
                flag = true;
                if (blob.hasCrossedFirstLine())
                    blob.setCrossedLine(1,index);
                else
                    blob.setCrossedLine(0,index);
            }
        }
    }
    return flag;
